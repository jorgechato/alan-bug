...
module.exports = {
  ...
  findUser : function(param,callback){
    ...
    User.find({$or:[{"email":{ $regex: re }},{"username":{ $regex: re }},{"role":{ $regex: re }}]}).sort({'date': 'desc'}).exec(function(err, user){
      if(!err && typeof callback == 'function') callback(user);
    });
  },
  ...
  findToken : function(param,callback){
    var re = new RegExp(param, 'i');

    Token.find({$or:[{"to":{ $regex: re }},{"from":{ $regex: re }}]}).sort({'date': 'desc'}).exec(function(err, token){
      if(!err && typeof callback == 'function') callback(token);
    });
  },

  deleteToken : function(id, callback){
    Token.remove({"_id": id}, function(){
      if(typeof callback == 'function') callback();
    });
  },
